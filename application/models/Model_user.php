...
class Model_user extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        self::$defaultMeta = array(
            ...
            "__activation_token" => array("type" => "string", "default" => "", "autoload" => 0, "protected" => 1),
            ...
            "is_del" => array("type" => "integer", "default" => 0, "autoload" => 1, "protected" => 1),
        );
    ...
    function uploadAvatar($uId, $iRemoveOld = 0)
    {
        $mInfo = $this->model_api->get('sh_users', array("avatar"), array("and" => array("uid" => $uId)));
        $iExt = strtolower(substr($_FILES['avatar']['name'], strrpos($_FILES['avatar']['name'], ".") + 1));
        $fileName = md5($uId . time()) . "." . $iExt;
        check_and_create_directory(USER_AVATAR_PATH . DS);
        move_uploaded_file($_FILES['avatar']['tmp_name'], USER_AVATAR_PATH . DS . $fileName);
        ...
        if (isset($mInfo['data'][0]['avatar']) && !empty($mInfo['data'][0]['avatar']))
            file_exists(USER_AVATAR_PATH . DS . $mInfo['data'][0]['avatar']) ? unlink(USER_AVATAR_PATH . DS . $mInfo['data'][0]['avatar']) : "";
        $iRes = success_res("success");
    ...
    function userDetailById($uId = 0)
    {
        ...
        if (isset($iUser['avatar']) && !empty($iUser['avatar']))
            $iUser['avatar'] = USER_AVATAR_URL . DS . $iUser['avatar'];
    ...
    function signUp($param)
    {
        ...
        $iUser = $this->model_api->get('sh_users', array("uid,name,email,mobile_number,created_date,modified_date,is_del"), array("and" => array("uid" => $uId)));
    ...
    function getsignUp($param)
    {
        ...
        $iQuery = "SELECT uid,name,email,mobile_number,avatar,address,is_del FROM sh_users {$iWhere} ORDER BY uid DESC {$iLimit}";
        ...
        if (isset($iCharacterList['data']) && count($iCharacterList['data']) > 0) {
            foreach ($iCharacterList['data'] as $key => $iCharacter) {
                ...
                if (isset($iCharacter['avatar']) && !empty($iCharacter['avatar']))
                    $iCharacterList['data'][$key]['avatar'] = USER_AVATAR_URL . DS . $iCharacter['avatar'];
            }
        }
    ...
    {
        $uId = user_id();
        $updateFields = array_intersect_key($param);

        $iUpdate = $this->model_api->update('sh_users', $updateFields, array("and" => array("uid" => $uId)));
